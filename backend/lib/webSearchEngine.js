...
const { createOpenRouterWebProvider } = require('./openRouterWebProvider');
...
async function executeWebGrounding({ provider, query, limit = 8 }, { allowExecution = false, context = {} } = {}) {
  ...
  const raw = await adapter.search({ query: normalizedQuery, limit: safeLimit }, context);
  if (Array.isArray(raw)) {
    return Object.freeze({
      evidence: '',
      sources: normalizeSources(raw.map(item => ({ ...item, type: 'web' }))),
      usage: {},
      provider,
      mode: 'search'
    });
  }
  if (!raw || typeof raw !== 'object') throw searchError('invalid_search_provider_result');
  return Object.freeze({
    ...raw,
    sources: normalizeSources((Array.isArray(raw.sources) ? raw.sources : []).map(item => ({ ...item, type: 'web' })))
  });
}

async function executeWebSearch(input, options = {}) {
  const grounded = await executeWebGrounding(input, options);
  return grounded.sources;
}

if (!registry.get(BUCKET, 'openrouter')) {
  registerSearchProvider('openrouter', createOpenRouterWebProvider());
}

module.exports = { registerSearchProvider, listSearchProviders, executeWebSearch, executeWebGrounding };
