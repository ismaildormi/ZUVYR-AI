...
(async()=>{
 await check('Frontend document',async()=>{const r=await request(app); const t=await r.text(); if(!r.ok) throw new Error(`HTTP ${r.status}`); if(!/ZUVYR/i.test(t)) throw new Error('ZUVYR marker missing'); return `HTTP ${r.status}`;});
