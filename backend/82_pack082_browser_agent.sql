...
begin
  ...
  insert into public.browser_agent_actions(
    ...
    action_type,action_fingerprint,target,input_sha256,input_length,input_text_redacted,
    ...
    p_input_sha256,p_input_length,p_input_text_redacted,p_risk,p_permission_action,
...
comment on table public.browser_agent_actions is
  'PACK082 idempotent browser actions. Only policy-approved non-sensitive typed text may be persisted owner-scoped; password, OTP, card and secret-like inputs are rejected before persistence.';
    ...
    if p_constraints ? 'actionFingerprint'
       and coalesce(p_constraints->>'actionFingerprint','') !~ '^[0-9a-f]{64}
