...
const stopStart=frontend.indexOf("const stopSession = async");
const stopEnd=frontend.indexOf("const wire = () =>",stopStart);
const stopSource=frontend.slice(stopStart,stopEnd);
assert(stopStart>=0&&stopEnd>stopStart);
assert(stopSource.indexOf("button.click()")<stopSource.indexOf("await finishSpeakingTurn(button, true)"));
assert(stopSource.includes("let stopConfirmed = !sessionId"));
assert(stopSource.includes("state.sessionId = sessionId"));
assert(stopSource.includes("Retry STOP"));
assert(stopSource.includes("Voice stopped locally — retry STOP"));
...
console.log('PASS: PACK073 FIX3 makes STOP local-first and retry-safe while preserving visible session authority after a server-stop failure');
