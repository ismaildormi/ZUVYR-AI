...
declare
  ...
  v_device public.ip_devices%rowtype;
  v_expected_hash text := lower(btrim(coalesce(p_expected_token_hash,'')));
  v_hash text := lower(btrim(coalesce(p_token_hash,'')));
begin
  if v_expected_hash !~ '^[0-9a-f]{64}$' then
  ...
  if v_hash = v_expected_hash then
    raise exception 'pack086_token_rotation_noop';
  end if;
  if p_token_expires_at is null
     or p_token_expires_at <= now()
     or p_token_expires_at > now() + interval '15 minutes' then
  ...
  if v_session.token_hash is null
     or v_session.token_hash <> v_expected_hash
