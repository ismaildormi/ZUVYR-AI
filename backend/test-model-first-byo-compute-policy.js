...
assert.equal(policy.execution_priority.active_priority_pack, '096');
...
assert.equal(roadmap.active_pack, '096');
...
assert.equal(roadmap.priority_override.active_priority_pack, '096');
assert.deepEqual(
...
assert.equal(state.active_pack, '096');
...
assert.equal(state.model_first_priority_override.active_priority_pack, '096');
assert.equal(
...
assert(continueHere.includes('PACK096 — ZUVYR 7 Manager / Operator — V1 Production Model + V2 Bridge is the active model-first Pack'));
...
console.log('PASS: PACK095 is engineering-finalized and PACK096 is the active model-first Pack');
