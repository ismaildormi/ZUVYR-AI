...
async function inspect(page,id,viewport,consoleErrors,networkErrors){
  const state=await page.evaluate(({id,viewport})=>{
    ...
    return {id,viewport,title:(el.querySelector('h1,h2,.feature-topbar .t,.modal-topbar .t')?.textContent||'').trim()||null,visibleText:visibleText.slice(0,50000),renderFailure:!visibleText||/^(null|undefined)$/i.test(visibleText),horizontalOverflow:Math.ceil(el.scrollWidth)>Math.ceil(rootRect.width)+3,scrollWidth:el.scrollWidth,clientWidth:Math.round(rootRect.width),overflowElements,scrollOffenders:visible.map(node=>({tag:node.tagName,id:node.id||null,className:String(node.className||'').slice(0,180),scrollWidth:node.scrollWidth,clientWidth:node.clientWidth,text:(node.textContent||'').trim().replace(/\s+/g,' ').slice(0,100)})).filter(x=>x.scrollWidth>x.clientWidth+3).sort((a,b)=>(b.scrollWidth-b.clientWidth)-(a.scrollWidth-a.clientWidth)).slice(0,40),duplicateIds,colorInventory:Object.entries(colors).sort((a,b)=>b[1]-a[1]).slice(0,80).map(([value,count])=>({value,count})),typography};
  },{id,viewport});
