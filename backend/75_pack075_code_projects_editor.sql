...
create table if not exists public.code_project_editor_state (
  ...
  open_files jsonb not null default '[]'::jsonb
    check (
      jsonb_typeof(open_files) = 'array'
      and jsonb_array_length(open_files) <= 20
    ),
  active_file text,
...
create table if not exists public.code_project_ai_edits (
  ...
  instruction_sha256 text not null
    check (instruction_sha256 ~ '^[0-9a-f]{64}$'),
  target_paths jsonb not null default '[]'::jsonb
    check (
      jsonb_typeof(target_paths) = 'array'
      and jsonb_array_length(target_paths) between 1 and 8
    ),
...
begin
  ...
  if exists (
    ...
    return 'pack075_file_contract_invalid';
  end if;

  if exists (
    select 1
    from (
      select lower(e->>'path') as path_key, count(*) as n
      from jsonb_array_elements(p_files) e
      group by lower(e->>'path')
    ) duplicate_paths
    where duplicate_paths.n > 1
  ) then
    return 'pack075_duplicate_file_path';
  end if;
  ...
  if v_total_bytes > 4194304 then
    return 'pack075_project_too_large';
  end if;
  ...
  if p_entry_file is not null and not exists (
    ...
    return 'pack075_entry_file_missing';
  end if;
...
begin
  ...
  v_validation :=
    public.validate_zuvyr_code_files_pack075(p_files, p_entry_file);
  ...
  insert into public.code_project_files(
    project_id,
    path,
    content,
    content_sha256,
  ...
  insert into public.code_project_editor_state(
    ...
    p_owner_id,
    case
      when p_entry_file is null then '[]'::jsonb
      else jsonb_build_array(p_entry_file)
    end,
    p_entry_file
  );
...
declare
  ...
  v_name text := btrim(coalesce(p_name, ''));
  v_reason text :=
    left(coalesce(nullif(btrim(p_reason), ''), 'manual_save'), 200);
  v_version_id uuid;
  v_snapshot jsonb;
...
begin
  ...
  if p_expected_revision is not null
     and v_project.revision <> p_expected_revision then
  ...
  delete from public.code_project_files existing_file
  where existing_file.project_id = p_project_id
    and not exists (
      select 1
      from jsonb_to_recordset(p_files)
        as incoming(path text, content text, sha256 text, language text)
      where lower(incoming.path) = lower(existing_file.path)
    );
...
begin
  ...
  from public.code_project_branches
  where project_id = p_project_id
    and name = v_project.current_branch;
...
begin
  ...
  from public.code_project_branches
  where project_id = p_project_id
    and name = btrim(coalesce(p_name, ''));
  ...
  where id = v_branch.head_version_id
  ...
  v_validation :=
    public.validate_zuvyr_code_files_pack075(v_files, v_entry_file);
  if v_validation is not null then
    raise exception '%', v_validation;
  end if;
...
comment on table public.code_project_editor_state is
  'PACK075 owner-scoped Code Studio UI state. No runtime or preview execution data is stored here.';
